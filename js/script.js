...
async function loadFromAPI() {
    try {
        ...
        const response = await apiFetch(`${API_BASE_URL}/api/properties?delYn=N`, {
            cache: 'no-cache',
            headers: {
...
async function loadDeletedProperties() {
    try {
        const response = await apiFetch(`${API_BASE_URL}/api/properties?delYn=Y`);
...
async function addAccount() {
    ...
    try {
        const response = await apiFetch(`${API_BASE_URL}/api/auth/accounts`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
...
async function loadAccounts() {
    try {
        ...
        if (accounts.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" class="empty-row">등록된 계정이 없습니다.</td></tr>';
            return;
        }
        ...
        tbody.innerHTML = accounts.map((account, index) => {
            ...
            const statusLabel = isDisabled
                ? '<span style="color:#e74c3c;font-weight:600;">비활성</span>'
                : '<span style="color:#27ae60;font-weight:600;">활성</span>';
            const isJemin = account.username === 'jemin';
            const canManage = currentUser === 'jemin' && !isJemin && account.username !== currentUser;

            return `
            <tr${isDisabled ? ' style="opacity:0.75;"' : ''}>
                <td>${index + 1}</td>
                ...
                <td>
                    ...
                    <button class="action-btn" onclick="toggleAccountStatus('${account.username}', true)" title="비활성화" style="margin-right:6px;">
                        <i class="fas fa-ban"></i> 비활성화
                    </button>
                    ` : ''}
                    ${canManage && isDisabled ? `
                    <button class="action-btn" onclick="toggleAccountStatus('${account.username}', false)" title="활성화" style="margin-right:6px;">
                        <i class="fas fa-check"></i> 활성화
                    </button>
                    ` : ''}
                    ${canManage ? `
                    <button class="action-btn delete" onclick="deleteAccount('${account.username}')" title="완전 삭제">
                        <i class="fas fa-trash"></i> 완전 삭제
                    </button>
...
// 계정 비활성화/활성화
async function toggleAccountStatus(username, disabled) {
    const action = disabled ? '비활성화' : '활성화';
    if (!confirm(`계정 "${username}"을(를) ${action}하시겠습니까?${disabled ? '\n\n비활성화 시 해당 계정의 모든 로그인이 즉시 종료됩니다.' : ''}`)) return;
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(data.error || `계정 ${action}에 실패했습니다.`);
        }

        showNotification(data.message || `계정이 ${action}되었습니다.`, 'success');
        loadAccounts();
    } catch (error) {
        console.error(`계정 ${action} 오류:`, error);
        showNotification(error.message, 'error');
    }
}
...
async function deleteAccount(username) {
    if (!confirm(`계정 "${username}"을(를) 완전히 삭제하시겠습니까?\n\n삭제된 계정은 복구할 수 없으며, 모든 로그인이 즉시 종료됩니다.`)) return;

    try {
        const response = await apiFetch(`${API_BASE_URL}/api/auth/accounts?username=${encodeURIComponent(username)}`, {
            method: 'DELETE'
        });
